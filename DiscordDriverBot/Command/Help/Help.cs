...
namespace DiscordDriverBot.Command.Help
{
    public class Help : TopLevelModule<HelpService>
    {
        ...
        public async Task Commands([Summary("模組名稱")] string module = null)
        {
            ...
            var embed = new EmbedBuilder().WithOkColor().WithTitle($"{cmds.First().Module.Name} 內包含的指令").WithFooter("輸入 `!!Help 指令` 以顯示指令的詳細說明，例 `!!Help Help`");
